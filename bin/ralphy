#!/bin/bash
# ralphy - Ralphy Wiggum Loop für autonome AI-Entwicklung
#
# Unterstützt drei CLI-Tools: claude, ccs, opencode
#
...
cleanup() {
    ...
    echo ""
    echo "🛑 Abbruch empfangen, räume auf..."
...
if [[ ! -f "$PROJECT_DIR/docs/prd.json" ]]; then
    ...
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Abbruch. Ralphy benötigt eine Konfiguration."
        exit 1
    fi
    ...
    if [[ -d "$TEMPLATE_DIR" ]]; then
        ...
        if [[ ! -f "$PROJECT_DIR/CLAUDE.md" ]]; then
            ...
            echo "⚠️  CLAUDE.md existiert bereits, überspringe..."
        fi
        ...
        echo "❌ FEHLER: Templates nicht gefunden unter $TEMPLATE_DIR"
        exit 1
    fi
fi
...
show_help() {
    ...
    echo ""
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║ Claude (Default) - Anthropic Subscription                        ║"
    echo "╠══════════════════════════════════════════════════════════════════╣"
    echo "║ ralphy 10                        # Claude direkt, 10 Iterationen  ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║ OpenCode Provider (--oc-*)                                       ║"
    echo "╠══════════════════════════════════════════════════════════════════╣"
    echo "║ --oc-gemini     Antigravity Gemini 3 Pro                        ║"
    echo "║ --oc-gemini-low Antigravity Gemini 3 Pro (low thinking)        ║"
    echo "║ --oc-flash      Antigravity Gemini 3 Flash (schneller)          ║"
    echo "║ --oc-agy        Antigravity Claude Sonnet 4.5                    ║"
    echo "║ --oc-agy-opus   Antigravity Claude Opus 4.5 + Thinking           ║"
    echo "║ --oc-glm        Z.AI GLM 4.6                                     ║"
    echo "║ --oc-glm7       Z.AI GLM 4.7 (newest)                            ║"
    echo "║ --oc-cerebras   Cerebras GLM 4.7 (fast inference)                ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║ CCS Provider (--ccs-*)                                           ║"
    echo "╠══════════════════════════════════════════════════════════════════╣"
    echo "║ --ccs-glm       Z.AI GLM                                         ║"
    echo "║ --ccs-glmt      Z.AI GLM + Thinking                              ║"
    echo "║ --ccs-agy       Antigravity                                      ║"
    echo "║ --ccs-gemini    Google Gemini                                    ║"
    echo "║ --ccs-codex     OpenAI Codex                                     ║"
    echo "║ --ccs-kimi      Kimi                                             ║"
    echo "║ --ccs-qwen      Qwen                                             ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo "  --list        Verfügbare Provider anzeigen"
...
show_providers() {
    echo "╔════════════════════════════════════════════════════════════════════╗"
    echo "║             Verfügbare Provider (verbundene Accounts)             ║"
    echo "╠════════════════════════════════════════════════════════════════════╣"
    echo "║                                                                    ║"
    echo "║ OPENCODE (opencode auth list):                                     ║"
    echo "║   Google (OAuth):      antigravity-gemini-3-pro, antigravity-gemini-3-flash ║"
    echo "║                        antigravity-claude-opus/sonnet-4.5         ║"
    echo "║   Z.AI Coding (API):   glm-4.6, glm-4.7                           ║"
    echo "║   Cerebras (API):      zai-glm-4.7, qwen-3-235b                   ║"
    echo "║                                                                    ║"
    echo "║ CCS (ccs):                                                         ║"
    echo "║   OAuth:               gemini, codex, agy                          ║"
    echo "║   API-Key:             glm, glmt, kimi, qwen                       ║"
    echo "║                                                                    ║"
    echo "╚════════════════════════════════════════════════════════════════════╝"
}
...
echo "╔════════════════════════════════════════════════════════════╗"
echo "║     🔄 Ralphy Wiggum Loop v$VERSION                                ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
case $TOOL in
    claude)
        if ! command -v claude &> /dev/null; then
            echo "❌ FEHLER: claude ist nicht installiert!"
            exit 1
        fi
        ;;
    opencode)
        ...
        if command -v opencode &> /dev/null; then
            ...
            echo "❌ FEHLER: opencode ist nicht installiert oder nicht im PATH!"
            echo "Gefundener PATH: $PATH"
            echo "Installiere mit: curl -fsSL https://opencode.ai/install | bash"
            echo "Oder füge das Installationsverzeichnis zum PATH hinzu."
            exit 1
        fi
    ...
    ccs)
        if ! command -v ccs &> /dev/null; then
            echo "❌ FEHLER: ccs ist nicht installiert!"
...
for ((i=1; i<=MAX_ITERATIONS; i++)); do
    ...
    if [[ "$TOOL" != "opencode" ]]; then
        echo ""
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo "📍 Iteration $i von $MAX_ITERATIONS - $(date '+%Y-%m-%d %H:%M:%S')"
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo ""
    fi
...
### prd.json:
$(cat "$PRD_FILE" 2>/dev/null || echo "⚠️  Datei nicht gefunden: $PRD_FILE")

### progress.txt:
$(cat "$PROGRESS_FILE" 2>/dev/null || echo "⚠️  Datei nicht gefunden: $PROGRESS_FILE")

### CLAUDE.md:
$(cat "$CLAUDE_FILE" 2>/dev/null || echo "⚠️  Datei nicht gefunden: $CLAUDE_FILE")
...
${PERCEPTIONS}"
            ...
            draw_statusbar() {
                ...
                case "$phase" in
                    recherche)  phase_icon="🔍"; phase_text="\033[1;33mRecherche\033[0m" ;;
                    implement)  phase_icon="⚙️ "; phase_text="\033[1;32mImplementierung\033[0m" ;;
                    validate)   phase_icon="✅"; phase_text="\033[1;34mValidierung\033[0m" ;;
                    commit)     phase_icon="📦"; phase_text="\033[1;35mCommit\033[0m" ;;
                    *)          phase_icon="⏳"; phase_text="\033[1mArbeitet...\033[0m" ;;
                esac
                ...
                printf "\033[%d;1H\033[K\033[1;36m📍 Iteration %d/%d\033[0m - %s %b" "$line1" "$i" "$MAX_ITERATIONS" "$phase_icon" "$phase_text" >&2

                # Line 2: Timers
                printf "\033[%d;1H\033[K\033[90m⏱️  %-8s - 🕐 Gesamt: %s\033[0m" "$line2" "$iter_time" "$total_time" >&2

                # Line 3: Tools
                printf "\033[%d;1H\033[K\033[90m📊 Tools: %-3d - 📖 %-3d - ✏️  %-3d - 💻 %-3d - 🔧 %-3d\033[0m" "$line3" "$total" "$reads" "$edits" "$bashes" "$others" >&2
...
expect {
    -re "Permission required.*Allow once" {
        ...
        if {[regexp -nocase {Remove|Delete} \$line] && ![string match "*$PROJECT_DIR*" \$line]} {
            send_user "⚠️  Überspringe: Remove-Anfrage außerhalb des Projekts\\n"
            interact
            exp_continue
        }
...
EOF
            ...
            echo ""
            echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            echo "✅ OpenCode beendet (Exit Code: $EXIT_CODE)"
            echo "📊 Tool-Statistik: $FINAL_TOTAL total │ 📖 Read: $FINAL_READS │ ✏️ Edit: $FINAL_EDITS │ 💻 Bash: $FINAL_BASHES │ 🔧 Other: $FINAL_OTHERS"
            ...
            if [[ -n "$STATS_OUTPUT" ]]; then
                ...
                INPUT_TOKENS=$(echo "$STATS_OUTPUT" | grep -oP '│Input\s+\K[\d.]+[KM]?' || echo "?")
                OUTPUT_TOKENS=$(echo "$STATS_OUTPUT" | grep -oP '│Output\s+\K[\d.]+[KM]?' || echo "?")
                echo "💰 Tokens: ~$AVG_TOKENS avg │ In: $INPUT_TOKENS │ Out: $OUTPUT_TOKENS │ Kosten: $TOTAL_COST"
            fi
            echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

            # Debug: Show result length and first/last lines if result is not empty
            if [[ -z "$result" ]]; then
                echo "⚠️  Warnung: OpenCode hat keine Ausgabe zurückgegeben (leere Antwort)"
            else
                RESULT_LENGTH=${#result}
                echo "📊 Ausgabe-Länge: $RESULT_LENGTH Zeichen"
...
${PERCEPTIONS}"
    ...
    if [[ -n "$result" ]] && ([[ "$result" == *"Error:"* ]] || [[ "$result" == *"error"* ]] || [[ "$result" == *"FEHLER"* ]] || [[ "$result" == *"File not found"* ]]); then
        echo ""
        echo "⚠️  Warnung: Fehler in der Ausgabe erkannt. Weiter mit nächster Iteration..."
        echo ""
    elif [[ -z "$result" ]]; then
        echo ""
        echo "⚠️  Warnung: Keine Ausgabe von OpenCode erhalten (möglicherweise Timeout oder Fehler)."
        echo "   Weiter mit nächster Iteration..."
        echo ""
    fi
    ...
    if [[ "$result" == *"<promise>COMPLETE</promise>"* ]] && [[ "$result" != *"Error:"* ]] && [[ "$result" != *"FEHLER"* ]]; then
        ...
        echo ""
        echo "╔════════════════════════════════════════════════════════════╗"
        echo "║  ✅ PRD COMPLETE! Alle kritischen Features implementiert.  ║"
        echo "╚════════════════════════════════════════════════════════════╝"
        echo ""
        echo "Provider: $PROVIDER_DISPLAY"
    ...
    echo ""
    echo "⏳ Pause vor nächster Iteration ($PAUSE_SECONDS Sekunden)..."
    sleep $PAUSE_SECONDS
done
...
echo ""
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  ⚠️  Max Iterationen erreicht ($MAX_ITERATIONS)                      ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
echo "Provider: $PROVIDER_DISPLAY"
